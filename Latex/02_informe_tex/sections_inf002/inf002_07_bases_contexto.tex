...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{@{}NXcc@{}}
		...
		& \textbf{$\Delta P$ total escenario cargado (selección)} & \textbf{165 Pa} & \textbf{225 Pa} \\
		& $\Delta P$ total escenario limpio & 70 Pa & 95 Pa \\
		& Potencia al eje ($\eta$ = 0.55, provisional) & 0.320 kW (0.43 HP) & -- \\
		\bottomrule
	\end{tabularx}
\end{table}
...
Sobre el motor, la potencia al eje corregida al sitio es 0.320 kW (0.43 HP); se instala provisionalmente un motor de 0.75 HP TEFC con tratamiento anticorrosivo, lo que deja un margen del orden del 70 \% sobre el eje, suficiente para absorber el derateo por altitud (regla práctica NEMA MG-1: 3 \% por cada 500 m sobre 1 000 msnm, es decir, aproximadamente 9 \% a 2 558 msnm \cite{nema_mg1_derate,silvertip_altitude}) y el factor de servicio exigido por el ambiente corrosivo. La potencia definitiva del motor se confirma con la curva de catálogo del ventilador seleccionado.
