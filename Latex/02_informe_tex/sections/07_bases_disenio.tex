...
La Tabla \ref{tab:room_data} resume las características geométricas y los criterios de ventilación del laboratorio. El laboratorio se ubica dentro de una planta de hipoclorito de calcio, cuyo ambiente exterior es altamente corrosivo; el ventilador, las rejillas, el damper de alivio y los soportes expuestos deben ser en PRFV, acero inoxidable 316 o acero con recubrimiento epóxico, y el motor debe ser TEFC anticorrosivo.
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{@{}N>{\hsize=0.9\hsize}X>{\hsize=1.2\hsize}Xc>{\hsize=0.9\hsize}X@{}}
		...
		& Control de presurización & Damper de alivio calibrable (obligatorio) + sensor de $\Delta P$ & -- & Criterio de diseño \\
		\bottomrule
	\end{tabularx}
\end{table}
...
La Tabla \ref{tab:fan_criteria} presenta los criterios de diseño del ventilador de impulsión. El punto de trabajo en el sitio corresponde al escenario de filtración cargada ($\Delta P$ filtro 154 Pa + 25 Pa de offset de presurización + 11 Pa de rejillas = 190 Pa total); dado que las curvas de catálogo se publican a densidad estándar ($\rho$ = 1.2 kg/m$^3$), la selección comercial se realiza en el punto equivalente de catálogo 3 840 m$^3$/h a 260 Pa \cite{dml_informe_investigacion_2026,dml_hd_vent_001}.
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{@{}N>{\hsize=0.9\hsize}X>{\hsize=1.2\hsize}Xc>{\hsize=0.9\hsize}X@{}}
		...
		& Presión total en el sitio (filtro cargado) & 190 & Pa & Memoria de cálculo \\
		& Punto equivalente de catálogo ($\rho$ = 1.2 kg/m$^3$) & 260 & Pa & Corrección de densidad, $k$ = 0.733 \\
		& Presión total en el sitio (filtro limpio) & 95 & Pa & Memoria de cálculo \\
		& Punto equivalente de catálogo (limpio) & 130 & Pa & Corrección de densidad, $k$ = 0.733 \\
		& Eficiencia total supuesta & 0.60 & -- & Criterio de diseño \\
		& Potencia teórica de aire & 0.338 (0.45 HP) & kW & Ec. (\ref{eq:potencia}) \\
		& Motor instalado & 1.0 HP TEFC anticorrosivo, 440 V, 3$\phi$, 60 Hz & -- & Criterio de diseño / cliente \\
		& Velocidad de impulsión & 8.0 & m/s & Criterio de diseño \\
		& Área de boca de impulsión & 0.1333 & m$^2$ & Ec. (\ref{eq:area_vent}) \\
		& Diámetro equivalente (radio) & 412 (206.0) & mm & Ec. (\ref{eq:diametro}) \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{@{}NXcccc@{}}
		...
		\midrule
		& MERV 13-14 limpio & 59 & 95 & 0.169 & Operación inicial \\
		& MERV 13-14 cargado (diseño) & 154 & 190 & 0.338 & Punto de selección \\
		& HEPA H13 & 250--600 & 290--640 & 0.516--1.138 & Descartado (no aplica) \\
		\bottomrule
	\end{tabularx}
\end{table}
...
La Tabla \ref{tab:grille_criteria} presenta los criterios de diseño de las rejillas de exfiltración. A la densidad del sitio, el cierre de orificio ($C_d$ = 0.60) de las tres rejillas a 3.0 m/s sostiene naturalmente solo 11 Pa, por debajo del mínimo admisible de 12.5 Pa; el objetivo de +25 Pa se garantiza con el damper de alivio calibrable, componente obligatorio del sistema \cite{dml_hd_rej_001,dml_hd_inst_001}.
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{@{}N>{\hsize=0.9\hsize}X>{\hsize=1.2\hsize}Xc>{\hsize=0.9\hsize}X@{}}
		...
		& Presión sostenida natural ($C_d$ = 0.60) & 11 & Pa & Cierre de orificio, $\rho$ = 0.88 kg/m$^3$ \\
