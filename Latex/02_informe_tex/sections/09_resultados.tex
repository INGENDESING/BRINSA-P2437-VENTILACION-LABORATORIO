...
La Tabla \ref{tab:fan_power} resume los resultados de potencia para diferentes escenarios de presión total a las condiciones del sitio (Cajicá, $\rho$ = 0.88 kg/m$^3$). El escenario de diseño corresponde a 190 Pa, que incluye pérdidas por filtración MERV 13-14 cargada, el offset de presurización y las rejillas de exfiltración; el punto equivalente de catálogo (densidad estándar 1.2 kg/m$^3$) es 3 840 m$^3$/h a 260 Pa \cite{dml_hd_vent_001,dml_hd_filt_001}.
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{@{}NXllc@{}}
		...
		& & \textbf{(Pa)} & \textbf{(kW)} & \\
		\midrule
		& MERV limpio & 95 & 0.169 & Ec. (\ref{eq:potencia}) \\
		& Diseño (MERV cargado) & 190 & 0.338 & Ec. (\ref{eq:potencia}) \\
		& Alto (+50 \% sobre diseño) & 285 & 0.507 & Ec. (\ref{eq:potencia}) \\
...
El modelo de dinámica de fluidos computacional (Autodesk CFD 2020) se resolvió con las condiciones de contorno de la Tabla \ref{tab:cfd_boundaries}. La Figura \ref{fig:cfd_vertical} presenta el campo de magnitud de velocidad en el plano vertical longitudinal; la Figura \ref{fig:cfd_horizontal}, en el plano horizontal. En ambos planos la velocidad máxima se sitúa en torno a \SI{8}{m/s}, consistente con la velocidad de impulsión de diseño, y se observa el patrón de recirculación en las zonas alejadas de la inyección.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.92\textwidth]{figures/cfd_caso1.png}
	\caption{Campo de magnitud de velocidad en el plano vertical longitudinal (vista $-Y$). Simulación CFD (Autodesk CFD 2020); escala 0--8201~mm/s ($\approx$ \SI{8.2}{m/s} en el máximo), consistente con la velocidad de impulsión de diseño. Se aprecia la inyección lateral y la recirculación en las zonas periféricas del recinto.}
	\label{fig:cfd_vertical}
\end{figure}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.92\textwidth]{figures/cfd_caso2.png}
	\caption{Campo de magnitud de velocidad en el plano horizontal (vista $+Z$). Simulación CFD (Autodesk CFD 2020); escala 0--7973~mm/s. Flujo desde la impulsión inferior izquierda hacia las rejillas del lateral derecho, con vórtices en los recintos secundarios.}
	\label{fig:cfd_horizontal}
\end{figure}
