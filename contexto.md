...
## Estado actual
- Última tarea completada (2026-09-17, sesión 2): **INF-001 emitido en REV1** (fecha 10/09/2026) por la reubicación de las rejillas. Tabla de control de revisiones con REV0 + REV1 (fila REV1 condicional `\ifnum\docRevision>0`, porque `00_hojafirmas.tex` y `00_portada.tex` son compartidas con INF-002, que sigue en REV0); encabezado corporativo «REV 1 / 10 de septiembre de 2026»; fechas de firma 10/09/2026; `codificacion.md` actualizado (INF-001 → Rev. 1). Re-emitidos los 7 entregables; verificado por extracción de texto del PDF (INF-001 REV1, INF-002 intacto en REV0).
- Tarea previa (2026-09-17, sesión 1): **actualización de INF-001 con la nueva simulación CFD — rejillas apiladas verticalmente**. BRINSA definió las 3 rejillas de exfiltración en posición vertical, una sobre otra, en la pared opuesta a la inyección; todo lo demás del diseño sin cambios. Se analizaron las 8 gráficas nuevas subidas a `Latex/02_informe_tex/figures/` (4 variables × 2 vistas; las de sufijo " 1" renombradas a `*_b.png`), se incluyeron las 8 figuras en `09_resultados.tex` con leyendas nuevas, y se reescribieron los comentarios CFD en `10_analisis.tex`, `11_conclusiones.tex`, `12_recomendaciones.tex` (disposición fija validada), `02_resumen.tex` y `13_anexos.tex`. También: `HD-REJ-001` §1.2 (disposición) + corrección de encabezado a Revisión 0, `docs/index.html` (tarjeta de rejillas) y `bases_diseno.yaml` (`rejillas_exfiltracion.ubicacion`). Emitidos los 7 entregables. Push a GitHub: commit `1c3650b`.
- Tarea previa (2026-07-28): **geometría Excel fija + REV0 única**. (i) Por instrucción del cliente, todas las hojas de contenido de los 5 libros Excel quedaron con ancho de columna 35 y alto de fila 50 (portadas y encabezado corporativo filas 1-7 intactos); cambio centralizado en `scripts/estilos_excel.py`. (ii) **Todos los entregables quedaron en Revisión CERO (0)** conforme a GP-N-09 (el cliente no ha recibido ninguna entrega): eliminadas las referencias REV1/REV2 de HD-VENT/FILT-001, listado_equipos.md, informe_investigacion.md, INF-001/INF-002 (tablas de control de revisiones solo con REV0, macros \descRevUno… eliminadas), `pdf_dts001.py`, `emitir.py`, `codificacion.md`, `docs/index.html`, `bases_diseno.yaml`. Re-emitidos los 7 entregables + PDF DTS-001; verificación automática: 0 coincidencias REV1/REV2 en `Emisiones/`. **Nota 2026-09-17: esta decisión quedó parcialmente superada — INF-001 pasó a REV1; los demás entregables siguen en REV0.**
...
## Bases de diseño congeladas (vigentes desde 2026-07-27; revisión documental: REV1 en INF-001 desde 2026-09-17, CERO (0) en los demás entregables)
...
- **SIN presurización** del cuarto; descarga libre a atmósfera por 3 rejillas 353×336 mm **apiladas verticalmente en la pared opuesta a la inyección** (disposición BRINSA, validada por CFD 2026-09-17)
...
- **INF-001 a REV1 (2026-09-17):** por la reubicación de las rejillas de exfiltración (apiladas verticalmente), INF-001 se emitió en Revisión 1 con fecha 10/09/2026. La fila REV1 de las tablas de control de revisiones es condicional (`\ifnum\docRevision>0`) porque `00_hojafirmas.tex` y `00_portada.tex` son compartidas con INF-002 (sigue en REV0). Decisión: `vault/03_Decisiones/2026-09-17_inf001-rev1-rejillas.md`.
- **Rejillas apiladas verticalmente, validadas por CFD (2026-09-17):** BRINSA definió la reubicación de las 3 rejillas de exfiltración en posición vertical, una sobre otra, en la pared opuesta a la inyección. Se re-ejecutó el modelo CFD (Autodesk CFD, Case 1) con esa disposición: el chorro de inyección alimenta directamente la rejilla central y las recirculaciones superior e inferior distribuyen el caudal a las rejillas de los extremos (~3 m/s en los tres niveles, sin retorno a la inyección ni estancamiento severo). INF-001 quedó actualizado con 8 figuras CFD (4 variables × 2 vistas) y los comentarios reescritos; la antigua recomendación de "paredes opuestas o perpendiculares" se reemplazó por la disposición fija. Decisión: `vault/03_Decisiones/2026-09-17_rejillas-verticales-cfd.md`.
