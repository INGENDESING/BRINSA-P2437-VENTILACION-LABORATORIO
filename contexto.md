...
## Estado actual
- Última tarea completada (2026-09-17): **actualización de INF-001 con la nueva simulación CFD — rejillas apiladas verticalmente**. BRINSA definió las 3 rejillas de exfiltración en posición vertical, una sobre otra, en la pared opuesta a la inyección; todo lo demás del diseño sin cambios. Se analizaron las 8 gráficas nuevas subidas a `Latex/02_informe_tex/figures/` (4 variables × 2 vistas; las de sufijo " 1" renombradas a `*_b.png`), se incluyeron las 8 figuras en `09_resultados.tex` con leyendas nuevas, y se reescribieron los comentarios CFD en `10_analisis.tex`, `11_conclusiones.tex`, `12_recomendaciones.tex` (disposición fija validada), `02_resumen.tex` y `13_anexos.tex`. También: `HD-REJ-001` §1.2 (disposición) + corrección de encabezado a Revisión 0, `docs/index.html` (tarjeta de rejillas) y `bases_diseno.yaml` (`rejillas_exfiltracion.ubicacion`). INF-001 compilado: 0 errores, 28 páginas; emitidos los 7 entregables con `scripts/emitir.py` (la primera corrida falló por un archivo de `Emisiones/` abierto; la reejecución completó).
